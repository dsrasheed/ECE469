...
boot_alloc(uint32_t n)
{
	...
	if (n > 0) {
		...
		if ((unsigned int) new_nextfree < (unsigned int) end)
			panic("boot_alloc: Memory allocation request is too large.\n");
...
inline void mark_pages_inuse(size_t start_i, size_t end_i) {
	while (start_i < end_i) pages[start_i++].pp_ref = 1;
}
//
...
page_init(void)
{
	...
	// Mark regions of memory that can never be allocated by the OS
	// Interrupt Table & BIOS Structures
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref != 0)
		panic("page_free: page %d has a nonzero ref count\n", (pp - pages) >> PGSHIFT);
	if (pp->pp_link != NULL)
		panic("page_free: page %d is already free\n", (pp-pages) >> PGSHIFT);
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	if (pte_p == NULL) return -E_NO_MEM;

	pp->pp_ref += 1;
	if (*pte_p & PTE_P) page_remove(pgdir, va);
