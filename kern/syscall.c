...
sys_env_set_status(envid_t envid, int status)
{
	...
	if (status != ENV_RUNNABLE && status != ENV_NOT_RUNNABLE)
		panic("sys_env_set_status: status %d is not ENV_RUNNABLE \
			 	or ENV_NOT_RUNNABLE", status);

	if ((err = envid2env(envid, &e, true)) < 0)
		return err;
	e->env_status = status;
	return 0;
}
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	// Obtain env structs from ids and check if valid access
	if ((err = envid2env(srcenvid, &src_e, 1)) < 0)
		return err;
	if ((err = envid2env(dstenvid, &dst_e, 1)) < 0)
		return err;
	
	// check srcva and dstva validity
	...
	// check va mapped in src environment
	pp = page_lookup(src_e->env_pgdir, srcva, &pte);
	if (pte == NULL || !(*pte & PTE_P))
		return -E_INVAL;
	
	// check src mapping is writable if permission requires writability
	if ((perm & PTE_W) && !(*pte & PTE_W))
		return -E_INVAL;
	
	// add page to dst environment
	if ((err = page_insert(dst_e->env_pgdir, pp, dstva, perm)) < 0)
		return err;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if (srcva < (void*) UTOP) {
		...
		if (e2->env_ipc_dstva < (void*) UTOP) {
			if (page_insert(e2->env_pgdir, pp, e2->env_ipc_dstva, perm) < 0)
				return -E_NO_MEM;
